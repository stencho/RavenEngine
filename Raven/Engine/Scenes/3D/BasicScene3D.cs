...
public class BasicScene : Scene {
    ...
    public override void BuildVisibilityLists(Camera camera) {
        foreach (var e in entities) {
            ...
            bool camera_hit = false;
            
            ent.Components.ForAllComponentsWithFlag(ComponentFlags.Render, (component) => {
                if (!camera_hit && component.GetBounds().Value.Intersects(camera.frustum)) {
                    entity_visibility_list.Add(new EntityVisibility(ent, camera));
                    camera_hit = true;
                }
            });
        }

        //render_list_deferred = render_list_deferred.OrderByDescending(a => a.distance).ToList();
    }
}