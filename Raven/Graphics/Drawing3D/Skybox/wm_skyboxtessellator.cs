...
public class SkyBoxTesselator {
    ...
    /// </summary>
    public void PrivateCreateSkyboxFromCrossImage(out VertexPositionColorNormalTexture[] vertices,
        out int[] indices, float scale, int Front, int Back, int Right, int Left, int Top, int Bottom) {
        VertexPositionColorNormalTexture[] vertices_sborss = new VertexPositionColorNormalTexture[24];
        ...
        for (int j = 0; j < 6; j++) {
            ...
            vertices_sborss[face + 0].TextureCoordinate = new Vector2(uvStart.X, uvStart.Y);
            vertices_sborss[face + 1].TextureCoordinate = new Vector2(uvStart.X, uvEnd.Y);
            vertices_sborss[face + 2].TextureCoordinate = new Vector2(uvEnd.X, uvStart.Y);
            vertices_sborss[face + 3].TextureCoordinate = new Vector2(uvEnd.X, uvEnd.Y);
        }
    ...
    public void Subdivide(VertexPositionColorNormalTexture[] source_vertices, int[] source_indices,
        out VertexPositionColorNormalTexture[] dest_vertices, out int[] dest_indices, int numberOfSubdivisions,
        float distance) {
    ...
    public void Subdivide(VertexPositionColorNormalTexture[] source_vertices, int[] source_indices, Vector3 origin,
        float distance, int numberOfSubdivisions, out VertexPositionColorNormalTexture[] dest_vertices,
        out int[] dest_indices) {
        VertexPositionColorNormalTexture[] sv = source_vertices;
        ...
        VertexPositionColorNormalTexture[] dv = new VertexPositionColorNormalTexture[d_totalVertices];
        ...
        for (int f = 0; f < Faces; f++) {
            ...
            for (int j = 1; j < d_verticesPerDirection - 1; j++) {
                ...
                dv[dindex] = LerpSubDivisionVertices(dv[d_lt_Edge], dv[d_lb_Edge], lerptime, distance, origin);
            }
            ...
            for (int j = 1; j < d_verticesPerDirection - 1; j++) {
                ...
                dv[dindex] = LerpSubDivisionVertices(dv[d_rt_Edge], dv[d_rb_Edge], lerptime, distance, origin);
            }
            ...
            for (int down = 0; down < d_verticesPerDirection; down++) {
                ...
                for (int across = 1; across < d_verticesPerDirection - 1; across++) {
                    ...
                    dv[creationindex] =
                        LerpSubDivisionVertices(dv[fromLeft], dv[toRight], lerptime, distance, origin);
                }
            }
        }
    ...
    /// </summary>
    public VertexPositionColorNormalTexture LerpSubDivisionVertices
    (
        VertexPositionColorNormalTexture from,
        VertexPositionColorNormalTexture to,
        float lerpPercentage,
        float distance,
        Vector3 origin
    ) {
        VertexPositionColorNormalTexture B = new VertexPositionColorNormalTexture();
        B.Color = new Color(
        ...
        B.TextureCoordinate = from.TextureCoordinate +
                              ((to.TextureCoordinate - from.TextureCoordinate) * lerpPercentage);
        return B;
    }
