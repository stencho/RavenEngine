...
public static class State {
    ...
    internal static Input main_thread_input => engine_binds.input;
    internal static Input update_thread_input => binds.input;
    ...
    public static void Update(GameTime gt) {
        ...
        if (engine_binds.pressed("click_right")) {
            ...
            camera.orientation *= Matrix.CreateRotationX((main_thread_input.mouse_delta.Y / GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Height) * 5);
            camera.orientation *= Matrix.CreateFromAxisAngle(Vector3.Up, -(main_thread_input.mouse_delta.X / GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Width) * 5);
        } else {
            Input.mouse_lock = false;
        }
        
        if (engine_binds.pressed("forward")) movement += camera.orientation.Forward;
        if (engine_binds.pressed("backward")) movement += camera.orientation.Backward;
    ...
    public static void Render() {
        ...
        Renderer.build_lighting(null, camera, buffer);
